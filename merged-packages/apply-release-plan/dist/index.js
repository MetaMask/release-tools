...
import { defaultDetectOrder, detect as detectFormatter, format, } from "@changesets/format";
...
export async function applyReleasePlan(releasePlan, packages, config = defaultConfig, snapshot, contextDir = import.meta.dirname) {
    ...
    const releasesWithPackage = releases.map((release) => {
        ...
        if (!pkg)
            throw new Error(`Could not find matching package for release of: ${release.name}`);
        return {
            ...release,
            ...pkg,
        };
    ...
    const dependencyUpdateOptions = {
        ...
        onlyUpdatePeerDependentsWhenOutOfRange: config.___experimentalUnsafeOptions_WILL_CHANGE_IN_PATCH
...
async function getNewChangelogEntry(releasesWithPackage, changesets, config, cwd, contextDir) {
    if (!config.changelog) {
        return Promise.resolve(releasesWithPackage.map((release) => ({
            ...release,
            changelog: null,
        })));
    }
    ...
    if (typeof possibleChangelogFunc.getReleaseLine === "function" &&
